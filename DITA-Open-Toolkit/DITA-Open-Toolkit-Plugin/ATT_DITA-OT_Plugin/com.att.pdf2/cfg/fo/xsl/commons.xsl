...
-->
  ...
  <xsl:template match="*" mode="processUnknowTopic"
    ...
    <xsl:choose>
      <xsl:when test="empty(ancestor::*[contains(@class,' topic/topic ')]) and 
                      empty(ancestor::ot-placeholder:glossarylist)">
         <!--
            DS modification:
            As of DITA-OT 3.6.1
            
            Standard out-of-the-box DITA-OT behavior causes each new topic to start on a 
            new page within the chapter. Another desired behavior is to have each topic 
            continue in the same page flow within the chapter, without any intervening 
            page breaks. By setting its value to “true()” or “false(),” the 
            “child-topics-of-a-chapter-start-on-a-new-page” toggle variable lets you 
            choose either one of these behaviors.
         -->
         <xsl:choose>
           <xsl:when test="boolean($child-topics-of-a-chapter-start-on-a-new-page)">
              <fo:page-sequence master-reference="{$page-sequence-reference}" xsl:use-attribute-sets="page-sequence.body">
                 ...
                 <fo:flow flow-name="xsl-region-body">
                    ...
                    Standard out-of-the-box DITA-OT behavior causes the booktitle and 
                    bookmeta content to appear on a kind of “front matter” within the 
                    generated PDF, which is why that content appears to be set off from 
                    the rest of the content, in a kind of pseudo title page. By setting 
                    the “show-booktitle-and-bookmeta-content-on-first-body-page” toggle 
                    variable to “true(),” you can change that behavior so that the 
                    booktitle and bookmeta content are processed together with the chapter 
                    content, all appearing within the “body” pages of the generated PDF. 
                    When you combine this toggle variable with the existing 
                    “generate-front-cover” toggle variable in 
                    com.att.pdf2/cfg/fo/attrs/basic-settings.xsl, you can make it so that 
                    the generated PDF has chapter content that immediately follows the 
                    booktitle and bookmeta content on the first page (by setting 
                    "generate-front-cover” to “false()” and 
                    “show-booktitle-and-bookmeta-content-on-first-body-page” to “true()”). 
                    With "generate-front-cover” set to “false()” and 
                    “show-booktitle-and-bookmeta-content-on-first-body-page” set to 
                    “true(),” even the header on the first page will disappear.
                    -->
                 	<xsl:if test="boolean($show-booktitle-and-bookmeta-content-on-first-body-page)">
           ...
           <xsl:otherwise>
              <xsl:if test="empty(preceding-sibling::*[contains(@class, ' topic/topic ')])">
                 <fo:page-sequence master-reference="{$page-sequence-reference}" xsl:use-attribute-sets="page-sequence.body">
                    <xsl:call-template name="startPageNumbering"/>
                    <xsl:call-template name="insertBodyStaticContents"/>
                    <fo:flow flow-name="xsl-region-body">
  ...
  <xsl:template match="*[contains(@class, ' topic/title ')]" mode="insertTopicHeaderMarkerContents">
    ...
    <!-- HFT Process bookmap chapters, prevent individual topics from starting new page -->
